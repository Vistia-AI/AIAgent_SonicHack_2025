...
def get_realtime_data():
    ...
    start_time=str(ts_now - 60*60*100)  # get 100 hours data
    end_time=str(ts_now)
    symbol = "SUSDT"
    klines = []
    ...
    coin_df["rsi_7"] = talib.RSI(coin_df["close"], timeperiod=7)
    coin_df["rsi_14"] = talib.RSI(coin_df["close"], timeperiod=14)
    df = coin_df[['open_time','open','high','low', 'close', 'volume', 'num_trades', 'rsi_7', 'rsi_14']].tail(24)  # last 24 hours
...
class ZerePyAgent:
    ...
    def _construct_system_prompt(self) -> str:
        ...
        if self._system_prompt is None:
            ...
            coin = "SUSDT"   
            real_time_data = get_realtime_data()
            # try:
            #     with engine.connect() as conn:
            #             query = f"SELECT * FROM proddb.f_coin_signal_1h WHERE symbol = '{coin}' and open_time > unix_timestamp(now()) - 3600*20 ORDER BY open_time DESC LIMIT 14"
            #             result = conn.execute(text(query))
            #             df = pd.DataFrame(result.fetchall(), columns=result.keys())

            #             if df.empty:
            #                 real_time_data += f"\n\n{coin}: No data available."
            #             else:
            ...
            system_prompt = f"""
                You are an AI assistant for Vistia, providing real-time AI-powered signals about the cryptocurrency market.
              
                You are an AI providing technical analysis for cryptocurrency.
                Analyze the current market trend for {coin}. 
                Based on real-time data from the last 14 time intervals:
                {real_time_data}
                Provide a technical summary including trend direction, RSI, moving averages, and support/resistance levels.
                """
            ...
            # self._system_prompt = "\n".join(prompt_parts)
            # self._system_prompt = prompt_parts.append(system_prompt)
